...
public partial class Form_SOAP_Template_StdSoapPageLite : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                if (JsongetPatientHistory.Data.PageSOAP.ToString().ToLower() == "00000000-0000-0000-0000-000000000000" || JsongetPatientHistory.Data.PageSOAP.ToString().ToLower() == "7ccd0a7e-9001-48ff-8052-ed07cf5716bb")
                {

                }
                else
                {
                    //jika user memilih choose template dari specialtypage
                    List<MarkerConfig> markerchoose = (List<MarkerConfig>)Session[Helper.SESSIONmarker];
                    string ischoose = markerchoose.Find(x => x.key == "IsChooseTemplate").value;
                    if (ischoose == "false")
                    {
                        Helper.ResponseRedirectSOAP(Request.QueryString["idPatient"], Request.QueryString["AdmissionId"], Request.QueryString["EncounterId"], Request.QueryString["PagefaId"], JsongetPatientHistory.Data.PageSOAP.ToString(), Request.QueryString["AppointmentId"], Request.QueryString["IsTele"]);
                    }
                    else if (ischoose == "true")
                    {
                        markerchoose.Find(x => x.key == "IsChooseTemplate").value = "false";
                        Session[Helper.SESSIONmarker] = markerchoose;
                    }
                }
                ...
                //Log.Debug(LogConfig.LogStart("getSOAP", logParam));
                ...
                //Log.Debug(LogConfig.LogEnd("getSOAP", Jsongetsoap.Status, Jsongetsoap.Message));
                //getrawat inap
                ...
                if (Session[Helper.Sessionsoapbackup + Request.QueryString["EncounterId"]] != null)
                {
                    ...
                    {
                        SOAP tempSOAPOriginal = Jsongetsoap.list;
                        Jsongetsoap.list.BackupDate = Jsongetsoap.list.ModifiedDate;

                        //versi session
                        SOAP tempSOAPBackup = (SOAP)Session[Helper.Sessionsoapbackup + Request.QueryString["EncounterId"]];
                ...
                hfsoapstring.Value = getsoap.Result.ToString().Replace("<", "< ");
                ...
                {
                    { "Organization_ID", Helper.organizationId.ToString() },
                    { "AdmissionType_ID", header.AdmissionTypeId.ToString() }
                };
                ...
                Session[Helper.SessionDataselectedDiagProc] = Helper.ToDataTable(Jsongetsoap.list.procedure_diagnosis.ToList());

                //Log.Debug(LogConfig.LogEnd());

                //#ITEM SPESIFIC ORDER SET TEMPLATE
                logParam = new Dictionary<string, string>
                {
                ...
                /// Diagnostik dan prosedur panel, grid sudah dipindahkan keluar dari modal
                //if (datadiagproc.Count > 0)
                //{
                ...
                List<InfectiousDisease> templistscreening = Jsongetsoap.list.infectious_disease.Where(x => x.patient_symptoms_id != Guid.Parse("00000000-0000-0000-0000-000000000000")).ToList();
                ...
                String[] tempArrayAlert = Jsongetsoap.list.infectious_alert.Where(x => x.patient_alert_id != Guid.Parse("00000000-0000-0000-0000-000000000000")).Select(x => x.alert_type_name).ToArray();
                if (tempArrayAlert.Length > 0)
                {
                    divKewaspadaan.Style.Add("display", "");
                    var infectiousAlert = String.Join(", ", tempArrayAlert);
                    listKewaspadaan.InnerText = infectiousAlert;
                }
                else
                {
                    divKewaspadaan.Style.Add("display", "none");
                }

                List<Subjective> tempTindakan = Jsongetsoap.list.subjective.Where(x => x.soap_mapping_id == Guid.Parse("f815de0e-e57a-4250-a2be-bdd27c1876ac")).ToList();
                if(tempTindakan.Count > 0)
				{
                    hdnTindakan.Value = new JavaScriptSerializer().Serialize(tempTindakan);
				}

                List<Subjective> tempDeleteReason = Jsongetsoap.list.subjective.Where(x => x.soap_mapping_id == Guid.Parse("0cbb6a1d-392b-4fed-8e8a-696bea3cbc32")).ToList();
                if (tempDeleteReason.Count > 0)
                {
                    hdnDeleteReason.Value = new JavaScriptSerializer().Serialize(tempDeleteReason);
                }

                List<InfectiousAlert> templistinfectiousalert = Jsongetsoap.list.infectious_alert.Where(x => x.patient_alert_id != Guid.Parse("00000000-0000-0000-0000-000000000000")).ToList();
                ...
                List<Objective> tempobjfallriskhandling = Jsongetsoap.list.objective.Where(y => y.soap_mapping_id == Guid.Parse("B0C9D8E0-7533-43CD-BFCF-E8C0AC1D4B7E")).ToList();
    ...
    public SOAP AssignDataFromSoapPage(SOAP soapmodel)
    {
        soapmodel.subjective.RemoveAll(x => x.soap_mapping_id == Guid.Parse("f815de0e-e57a-4250-a2be-bdd27c1876ac"));
        ...
        soapmodel.subjective.RemoveAll(x => x.soap_mapping_id == Guid.Parse("0cbb6a1d-392b-4fed-8e8a-696bea3cbc32"));
        ...
        soapmodel.objective.RemoveAll(x => x.soap_mapping_id == Guid.Parse("B0C9D8E0-7533-43CD-BFCF-E8C0AC1D4B7E"));
        ...
        soapmodel.objective.Find(y => y.soap_mapping_id == Guid.Parse("19516BE9-9D54-4E30-8E90-0A8B41F5BA66")).value = txtbmi.Text;
        ...
        soapmodel.planning.Find(y => y.soap_mapping_id == Guid.Parse("7198D4D5-7670-4853-9AFA-CD7C606C5C27")).value = "";
        soapmodel.planning.Find(y => y.soap_mapping_id == Guid.Parse("7198D4D5-7670-4853-9AFA-CD7C606C5C27")).remarks = hdnSchedule_travel.Value;
        soapmodel.planning.Find(y => y.soap_mapping_id == Guid.Parse("3893BF09-82A2-449A-B5F0-D3A2BF9905C1")).value = hdnCondition_travel.Value;
        soapmodel.planning.Find(y => y.soap_mapping_id == Guid.Parse("3893BF09-82A2-449A-B5F0-D3A2BF9905C1")).remarks = hdncondition_date.Value;
        soapmodel.planning.Find(y => y.soap_mapping_id == Guid.Parse("A4D139D7-67E2-44BC-A5E7-8BB17DD3F21E")).value = hdnSeating_type.Value;
        soapmodel.planning.Find(y => y.soap_mapping_id == Guid.Parse("A4D139D7-67E2-44BC-A5E7-8BB17DD3F21E")).remarks = "";
        soapmodel.planning.Find(y => y.soap_mapping_id == Guid.Parse("80A8E43A-53BF-4A32-B992-C9F32C7AA458")).value = hdnEscort_type.Value;
        soapmodel.planning.Find(y => y.soap_mapping_id == Guid.Parse("80A8E43A-53BF-4A32-B992-C9F32C7AA458")).remarks = hdnescort_ddl.Value;
        soapmodel.planning.RemoveAll(x => x.soap_mapping_id == Guid.Parse("F3378407-B8A6-45FF-A27B-87F3CCF0F2B4"));
        ...
        soapmodel.planning.Find(y => y.soap_mapping_id == Guid.Parse("30F9892A-22C7-4C8B-8005-BE08E105F05F")).value = "";
        soapmodel.planning.Find(y => y.soap_mapping_id == Guid.Parse("30F9892A-22C7-4C8B-8005-BE08E105F05F")).remarks = txtTravelRecommendation.Text.ToString();
    ...
    protected void btnsave_click(object sender, EventArgs e)
    {
        ...
        catch (Exception ex)
        {
            ...
            Log.Error(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "btnsave_click", StartTime, "Error", MyUser.GetUsername(), "", jsonfailed, ex.Message)); //Log.Error(LogConfig.LogError(ex.Message.ToString() + jsonfailed), ex);
    ...
    {
        string StartTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff"); //Log.Info(LogConfig.LogStart());
        btnFinalSubmitDisable.Enabled = false;
        btnFinalSubmitDisableTC.Enabled = false;
        ...
        catch (Exception ex)
        {
            ...
            Log.Error(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "btnSubmitDisable_click", StartTime, "Error", MyUser.GetUsername(), "", jsonfailed, ex.Message)); //Log.Error(LogConfig.LogError(ex.Message.ToString() + jsonfailed), ex);
            //ScriptManager.RegisterStartupScript(this, this.GetType(), "alertempty", "alert('Failed to Submit SOAP');", true);
        ...
        btnFinalSubmitDisable.Enabled = true;
        btnFinalSubmitDisableTC.Enabled = true;
        //Log.Info(LogConfig.LogEnd());
    }
    ...
    protected void btnSubmit_click(object sender, EventArgs e)
    {
        ...
        btnFinalSubmitTC.Enabled = false;
        ...
        catch (Exception ex)
        {
            ...
            Log.Error(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "btnSubmit_click", StartTime, "Error", MyUser.GetUsername(), "", jsonfailed, ex.Message)); //Log.Error(LogConfig.LogError(ex.Message.ToString() + jsonfailed), ex);
            //ScriptManager.RegisterStartupScript(this, this.GetType(), "alertempty", "alert('Failed to Submit SOAP');", true);
    ...
    public void Submit_SOAP_disable()
    {
        ...
        try
        {
            ...
            if (orgsetting.Find(y => y.setting_name.ToUpper() == "MANDATORY_SOAP".ToUpper()).setting_value == "TRUE")
            {
                if (Complaint.Text != "" && Anamnesis.Text != "" && txtPrimary.Text != "" && txtOthers.Text != "" && txtPlanning.Text != "")
                {
                    ...
                    //log.Debug(LogLibrary.Logging("S", "Submit_SOAP_disable", Helper.GetLoginUser(this), soapmodel.ToString()));
                    ...
                    //Log.Debug(LogConfig.LogStart("SubmitSOAP", logParam, LogConfig.JsonToString(soapmodel)));
                    var getMap = clsSOAP.SubmitSOAP(soapmodel, itemid, ConsultationAmount, discountamount, "-", hfPageSoapId.Value, hfitemname.Value, appointmentid, username);
                    ...
                    //Log.Debug(LogConfig.LogEnd("SubmitSOAP", Status, Message));
            ...
            else
            {
                ...
                soapmodel.objective.Find(y => y.soap_mapping_id == Guid.Parse("52ce9350-bfb2-4072-8893-d0c6cf8b3634")).value = txtweight.Text;
                soapmodel.objective.Find(y => y.soap_mapping_id == Guid.Parse("2a8dbddb-edfe-4704-876e-5a2d735bb541")).value = txtheight.Text;
                soapmodel.objective.Find(y => y.soap_mapping_id == Guid.Parse("7218971C-E89F-4172-AE3C-B7FB855C1D6D")).remarks = "Abnormal";
                soapmodel.objective.Find(y => y.soap_mapping_id == Guid.Parse("7218971C-E89F-4172-AE3C-B7FB855C1D6D")).value = txtOthers.Text;
                soapmodel.objective.Find(y => y.soap_mapping_id == Guid.Parse("A8E0013B-0443-4E7A-B670-4DB9362B40E4")).value = txtlingkarkepala.Text;
                soapmodel.objective.Find(y => y.soap_mapping_id == Guid.Parse("19516BE9-9D54-4E30-8E90-0A8B41F5BA66")).value = txtbmi.Text;
        ...
        catch (Exception ex)
        {
            ...
            Log.Error(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "Submit_SOAP_disable", StartTime, "Error", MyUser.GetUsername(), "", jsonfailed, ex.Message)); //Log.Error(LogConfig.LogError(ex.Message.ToString() + jsonfailed), ex);
    ...
    public void ActionSubmit()
    {
        ...
        /*if (chkScreen1.Checked)
        {
            ...
            soapmodel.subjective.Add(tempscreening);
        }
        ...
        //Log.Debug(LogConfig.LogStart("SubmitSOAP", logParam, LogConfig.JsonToString(soapmodel)));
        var getMap = clsSOAP.SubmitSOAP(soapmodel, itemid, ConsultationAmount, discountamount, "-", hfPageSoapId.Value, totalfeeformat[0].ToString(), appointmentid, username);
        if (getMap == null)
        {
            Log.Debug(LogConfig.LogStart("SubmitSOAPReturn", logParam, "Submit Response : Null"));
        }
    ...
    public void Submit_SOAP()
    {
        ...
        catch (Exception ex)
        {
            ...
            Log.Error(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "Submit_SOAP", StartTime, "Error", MyUser.GetUsername(), "", jsonfailed, ex.Message)); //Log.Error(LogConfig.LogError(ex.Message.ToString() + jsonfailed), ex);
            //ScriptManager.RegisterStartupScript(this, this.GetType(), "alertempty", "alert('Failed to Submit SOAP');", true);
            error_box("Failed to Submit SOAP", ex);
        }
    ...
    public void SaveDraft_SOAP()
    {
        ...
        try
        {
            ...
            if (hfsavemode.Value == "0")
            {
                ...
                //log.Debug(LogLibrary.Logging("S", "SaveDraft_SOAP", Helper.GetLoginUser(this), soapmodel.ToString()));
                //string appointmentid = Request.QueryString["AppointmentId"];
                //string username = Helper.GetLoginUser(this);
                ...
                //Log.Debug(LogConfig.LogStart("SaveAsDraftSOAP", logParam, LogConfig.JsonToString(soapmodel)));
                var getMap = clsSOAP.SaveAsDraftSOAP(soapmodel, hfPageSoapId.Value, appointmentid, username);
                var JsongetMap = (JObject)JsonConvert.DeserializeObject<dynamic>(getMap.Result);
                var Status = JsongetMap.Property("status").Value.ToString();
                var Message = JsongetMap.Property("message").Value.ToString();
                //Log.Debug(LogConfig.LogEnd("SaveAsDraftSOAP", Status, Message));
            ...
            ScriptManager.RegisterStartupScript(this, this.GetType(), "hidecopy", "hidecopy();", true);
            Log.Debug(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "SaveDraft_SOAP", StartTime, "OK", MyUser.GetUsername(), "", "", ""));
        }
        catch (Exception ex)
        {
            SOAP x = (SOAP)Session[Helper.Sessionsoapmodel + hfguidadditional.Value];
            ...
            Log.Error(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "SaveDraft_SOAP", StartTime, "Error", MyUser.GetUsername(), "", jsonfailed, ex.Message)); //Log.Error(LogConfig.LogError(ex.Message.ToString() + jsonfailed), ex);
            //ScriptManager.RegisterStartupScript(this, this.GetType(), "alertempty", "alert('Failed to Save SOAP');", true);
            error_box("Failed to Save SOAP", ex);
        }
        //Log.Info(LogConfig.LogEnd());
    ...
    protected void btngethopeprescription_onClick(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            //log.Debug(LogLibrary.Logging("S", "getSOAP", Helper.GetLoginUser(this), "param: " + hfcopyEncounterId.Value + "," + long.Parse(hfcopyPatientId.Value) + "," + long.Parse(hfcopyAdmissionId.Value) + "," + long.Parse(hfcopyOrganizationId.Value) + "," + long.Parse(hfcopyDoctorId.Value) + ")"));
    ...
    protected void btnCopySOAP_onClick(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (hfsoapcopystring.Value != "")
            {
                ...
                if (chkLab.Checked)
                {
                    foreach (var x in Jsoncopysoap.list.cpoe_trans.Where(x => x.type == "ClinicLab" || x.type == "CitoLab" || x.type == "MicroLab" || x.type == "PatologiLab" || x.type == "PanelLab" || x.type == "MDCLab"))
                    {
                ...
                {
                    foreach (var x in Jsonrawsoap.list.cpoe_trans.Where(x => x.type == "ClinicLab" || x.type == "CitoLab" || x.type == "MicroLab" || x.type == "PatologiLab" || x.type == "PanelLab" || x.type == "MDCLab"))
                    {
                        if (x.type == "ClinicLab" || x.type == "CitoLab" || x.type == "MicroLab" || x.type == "PatologiLab" || x.type == "PanelLab" || x.type == "MDCLab")
                        {
                            temp.Add(x);
                ...
                if (chkRad.Checked)
                {
                    foreach (var x in Jsoncopysoap.list.cpoe_trans.Where(x => x.type == "MRI1" || x.type == "MRI3" || x.type == "Radiology" || x.type == "USG" || x.type == "CT"))
                    {
                ...
                {
                    foreach (var x in Jsonrawsoap.list.cpoe_trans.Where(x => x.type == "MRI1" || x.type == "MRI3" || x.type == "Radiology" || x.type == "USG" || x.type == "CT"))
                    {
                        if (x.type == "MRI1" || x.type == "MRI3" || x.type == "Radiology" || x.type == "USG" || x.type == "CT")
                        {
                            temp.Add(x);
                ...
                if (chkDrugs.Checked)
                {
                    ...
                    foreach (var x in Jsoncopysoap.list.prescription.Where(x => x.compound_id == Guid.Parse("00000000-0000-0000-0000-000000000000") && x.is_consumables == 0 && x.is_delete == 0))
                    {
                ...
                {
                    foreach (var x in Jsonrawsoap.list.prescription.Where(x => x.compound_id == Guid.Parse("00000000-0000-0000-0000-000000000000") && x.is_consumables == 0 && x.is_delete == 0))
                    {
                        if (x.compound_id == Guid.Parse("00000000-0000-0000-0000-000000000000") && x.is_consumables == 0 && x.is_delete == 0)
                        {
                            drugstemp.Add(x);
                ...
                {
                    foreach (var x in Jsoncopysoap.list.prescription.Where(x => x.is_consumables == 1))
                    {
                ...
                {
                    foreach (var x in Jsonrawsoap.list.prescription.Where(x => x.is_consumables == 1))
                    {
                        if (x.is_consumables == 1)
                        {
                            drugstemp.Add(x);
    ...
    protected void ButtonAjaxSearchDiagProc_Dis_Click(object sender, EventArgs e)
    {
        ...
        //        DataTable dttemp = Helper.ToDataTable(data);
                ...
                /// this grid has been move to outside of modal
    ...
    //        // Update Panel
    //        UpdatePanel panelDiagnostic = (UpdatePanel)StdPlanning.FindControl("UpdatePanelDivDiagnostic");
    //        UpdatePanel panelDiagnosticFutureOrder = (UpdatePanel)StdPlanning.FindControl("UpdatePanelDivDiagnosticFutureOrder");
    //        UpdatePanel panelProcedure = (UpdatePanel)StdPlanning.FindControl("UpdatePanelDivProcedure");
    //        UpdatePanel panelProcedureFutureOrder = (UpdatePanel)StdPlanning.FindControl("UpdatePanelDivProcedureFutureOrder");
    //        UpdatePanel panelExistDiagProc = (UpdatePanel)StdPlanning.FindControl("updatepanelexistdiagproc");
    //        panelDiagnostic.Update();
    //        panelDiagnosticFutureOrder.Update();
    //        panelProcedure.Update();
    //        panelProcedureFutureOrder.Update();
    //        panelExistDiagProc.Update();


    //        Log.Debug(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "btndeletediagproc_dis_Click", StartTime, "OK", MyUser.GetUsername(), "", "", ""));
    //    }
    //    catch (Exception ex)
    //    {
    //        Log.Error(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "btndeletediagproc_dis_Click", StartTime, "Error", MyUser.GetUsername(), "", "", ex.Message)); //Log.Error(LogConfig.LogError(ex.Message.ToString()), ex);
    //        ScriptManager.RegisterStartupScript(Page, Page.GetType(), "connection", "alert('Connection Time Out. Please Try Again');", true);
    //    }
    //    //Log.Info(LogConfig.LogEnd());
    //}
    ...
    protected void NoDrugAllergy_Click(object sender, EventArgs e)
    {
        ...
        Log.Debug(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "NoDrugAllergy_Click", StartTime, "OK", MyUser.GetUsername(), "", "", ""));
        //Log.Info(LogConfig.LogEnd());
    }
    ...
    protected void NoFoodAllergy_Click(object sender, EventArgs e)
    {
        ...
        Log.Debug(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "NoFoodAllergy_Click", StartTime, "OK", MyUser.GetUsername(), "", "", ""));
    }
    ...
    protected void NoOtherAllergy_Click(object sender, EventArgs e)
    {
        ...
        Log.Debug(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "NoOtherAllergy_Click", StartTime, "OK", MyUser.GetUsername(), "", "", ""));
        //Log.Info(LogConfig.LogEnd());
    }
    ...
    protected void NoRoutineMedication_Click(object sender, EventArgs e)
    {
        ...
        Log.Debug(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "NoRoutineMedication_Click", StartTime, "OK", MyUser.GetUsername(), "", "", ""));
        //Log.Info(LogConfig.LogEnd());
    }
    ...
    protected void NoAllAllergy_Click(object sender, EventArgs e)
    {
        ...
        Log.Debug(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "NoAllAllergy_Click", StartTime, "OK", MyUser.GetUsername(), "", "", ""));
        //Log.Info(LogConfig.LogEnd());
    }
    ...
    protected void ButtonGetBackup_Click(object sender, EventArgs e)
    {
        ...
        Log.Debug(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "ButtonGetBackup_Click", StartTime, "OK", MyUser.GetUsername(), "", "", ""));
    ...
    protected void ButtonGetOri_Click(object sender, EventArgs e)
    {
        ...
        Log.Debug(LogLibrary.SaveLogNew(Helper.organizationId.ToString(), "EncounterId", hfEncounterId.Value.ToString(), "ButtonGetOri_Click", StartTime, "OK", MyUser.GetUsername(), "", "", ""));
        //Log.Info(LogConfig.LogEnd());
    }
    ...
    protected void btnSaveReferal_Click(object sender, EventArgs e)
    {
        try {
            ...
            //TextBox txtReferralremarks = (TextBox)ModalReferal.FindControl("TBreferral_remark");
            //List<ReferalData> newReferaldatas = new List<ReferalData>();
            //var temp = referalDatas.Where(x => x.referral_doctor_id > 0 || x.speciality_id > 0 || !string.IsNullOrEmpty(x.external_referral_to)).ToList();
