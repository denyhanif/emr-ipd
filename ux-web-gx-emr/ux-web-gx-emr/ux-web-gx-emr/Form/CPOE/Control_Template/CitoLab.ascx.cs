...
public partial class Form_CPOE_Control_Template_CitoLab : System.Web.UI.UserControl
{
    ...
    public void GetMappingCitoLab(List<CpoeMapping> listmap, string guidadditional)
    {
        DataTable dt;
        try
        {
            ...
            if (Session[Helper.SessionLabPathologyChecked + hfguidadditional.Value] != null)
            {
                ...
                if (listmapping.Count > 0)
                {
                    var data = (
                                ...
                                from b in joined.Where(x => x.iscito == 1 && x.isdelete == 0).DefaultIfEmpty()
