...
public partial class Form_CPOE_Control_Template_RptCitoLab : System.Web.UI.UserControl
{
    ...
    public event customHandler checkIfExistCito;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //    setENG = "";
        //    setIND = "none";
        //    HFisBahasa_citolab.Value = "ENG";
        //}
        //else if (Session[Helper.SessionLanguage] == "2")
        //{
        //    setENG = "none";
        //    setIND = "";
        //    HFisBahasa_citolab.Value = "IND";
        //}
        //else
        //{
        //    setENG = "";
        ...
        //set bahasa
        //ScriptManager.RegisterStartupScript(this, GetType(), "bahasa", "switchBahasa_citolab();", true);

        //if (!IsPostBack)
        //{
        //    //GetMappingCitoLab(2, "CitoLab");
        //}
    }

    public void InitializeNotes(CpoeNotes cpoenotes)
    {
        txtCitoNotes.Text = cpoenotes.notes;
    }

    public List<CpoeNotes> getvaluesnotes(List<CpoeNotes> cpoenotes)
    {
        //bool alreadyExists = cpoenotes.Any(x => x.notes_type == "CitoLab");
        if (cpoenotes.Any(x => x.notes_type == "CitoLab"))
        {
            foreach (CpoeNotes tempvalues in cpoenotes)
            {
                if (tempvalues.notes_type == "CitoLab")
                {
                    tempvalues.notes = txtCitoNotes.Text;
                }
            }
        }
        else
        {
            ...
            tempnotes.notes_type = "CitoLab";
            tempnotes.notes = txtCitoNotes.Text;
            cpoenotes.Add(tempnotes);
        }
        return cpoenotes;
    }

    public void GetMappingCitoLab(List<CpoeMapping> listmap, string guidadditional, string flagFO)
    {
                ...
                if (listmapping.Count > 0)
                {
                    if (flagFO == "true")
                    {
                        data = (
                                ...
                                from b in joined.Where(x => x.iscito == 1 && x.isdelete == 0 && x.IsFutureOrder == true).DefaultIfEmpty()
                                where a.type == "CitoLab" && a.organizationId != 0
                                select new LabList
                                {
                    ...
                    else if (flagFO == "false")
                    {
                        data = (
                                from a in listmapping
                                join b in listchecked on a.item_id equals b.id into joined
                                from b in joined.Where(x => x.iscito == 1 && x.isdelete == 0 && x.IsFutureOrder == false).DefaultIfEmpty()
                                where a.type == "CitoLab" && a.organizationId != 0
                                select new LabList
                                {
                                    organizationId = a.organizationId,
